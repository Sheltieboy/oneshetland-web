...
export const LAUNCH_READINESS: ReadinessDataset = {
  ...
  items: [
    ...
    },
    {
      ...
      status: "needs_verification", criticality: "important", weight: 3,
      evidence: "Found 25 Sep while re-auditing the Google Maps key. The browser key is now HTTP-referrer restricted (good, and the reason the old 'unrestricted' finding is stale), but web and mobile use the SAME key (EAS production EXPO_PUBLIC_GOOGLE_PLACES_KEY equals the web key). The app's address search (react-native-google-places-autocomplete: Fetch pickup/drop-off, event create, business register, shift post) calls the Places REST API from the device, which sends no referrer, and Google answers those calls with REQUEST_DENIED ('API keys with referer restrictions cannot be used with this API') — proven with curl. So address search in the app is probably failing today. NOT verified in the app itself, and it may have a manual-entry fallback. The Android native Maps SDK also uses this key.",
      nextAction: "1) In the app, try address search (Fetch request step 2, event create). 2) If it fails, in the Google Cloud console create a SEPARATE mobile key: Places API only, restricted to the iOS bundle id (send it as X-Ios-Bundle-Identifier) and, for the Android Maps SDK, package name + SHA-1; keep the web key referrer-restricted. 3) Set it as EXPO_PUBLIC_GOOGLE_PLACES_KEY in the EAS production environment and ship (JS-only for Places; a native build for the Android Maps key).",
      lastUpdated: "2026-09-25",
    },
    {
      ...
      nextAction: "Schedule after launch; item 5 is quick and worth doing first.",
      lastUpdated: "2026-09-25",
    },
    {
