...
export function SpikSprint({ userId }: { userId: string | null }) {
  ...
  const end = useCallback(async () => {
    ...
    setPhase("done");
    setScore((s) => { if (userId && s >= 0) submitScore(userId, "spik_sprint", s, { durationMs: 60000, metadata: { best_streak: bestStreak }, xpEarned: s }).catch(() => {}); return s; });
  }, [userId, bestStreak]);
    ...
    return (
      <Centered>
        ...
        <div className="rounded-card border border-line bg-paper p-8 text-center shadow-soft">
          ...
          {userId ? <p className="mt-2 text-sm font-semibold" style={{ color: ACCENT }}>+{score} XP</p> : <p className="mt-2 text-xs text-ink-muted"><a href="/sign-in?next=/games/spik-sprint" className="underline">Sign in</a> to save your score</p>}
