...
export const metadata = { title: "Memories · OneShetland", description: "A living map of the islands — pin a place, tell its story, leave a photo or a voice note." };
    ...
    <>
      ...
      <section className="relative isolate overflow-hidden text-paper" style={{ background: MEMORIES }}>
        ...
        <div className="relative mx-auto max-w-6xl px-5 py-12 sm:py-16">
          ...
          <h1 className="mt-2 font-display text-5xl font-bold sm:text-6xl">Memories</h1>
          ...
          <Link href="/memories/new" className="mt-5 inline-block rounded-pill bg-white px-6 py-3 font-semibold shadow-soft transition hover:brightness-95" style={{ color: MEMORIES }}>+ Add a memory</Link>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-6xl px-5 py-10 sm:py-12 space-y-10">
        ...
        <section>
          ...
          <p className="mt-2 text-sm text-ink-muted">{pins.length} memories pinned across Shetland. Tap a marker to read it.</p>
        </section>
        ...
        <section>
          <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
            ...
            <form action="/memories" method="get" className="flex gap-2">
              <input name="q" defaultValue={q ?? ""} placeholder="Search memories, places, eras…" className="rounded-pill border border-line bg-paper px-4 py-2 text-sm text-ink shadow-soft outline-none placeholder:text-ink-faint" />
          ...
          {feed.length === 0 ? (
            <div className="rounded-card border border-dashed border-line bg-paper/60 px-6 py-12 text-center">
              <p className="font-display text-lg font-bold text-ink">No memories yet</p>
              <p className="mt-1 text-sm text-ink-muted">Be the first to pin a story to the map.</p>
              <Link href="/memories/new" className="mt-4 inline-block rounded-pill px-5 py-2.5 text-sm font-semibold text-white" style={{ background: MEMORIES }}>Add a memory</Link>
            </div>
          ) : (
