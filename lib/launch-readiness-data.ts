...
export const LAUNCH_READINESS: ReadinessDataset = {
  ...
  items: [
    ...
    },
    {
      ...
      evidence: "Verified after payment 25 Sep: order paid, one valid ticket, inventory incremented once, PaymentIntent succeeded, destination transfer and application fee correct, webhooks processed once. PHYSICAL SCAN 25 Sep 00:26 UTC: the organiser scanned it on mobile and it went valid → used correctly (audit trail: one 'valid' check-in, two later scans refused as 'already_used', no duplicates). GAP FOUND: the attendee saw only 'Used', with no celebration. Root cause: the celebration existed and was wired, but fired only from a Realtime push that landed while the ticket screen was mounted; a phone locked or showing the QR missed it, and the screen then loaded an already-used row with no memory of the transition. FIXED + DEPLOYED 25 Sep (mobile 4695440, iOS OTA group ee23314f, runtime 990f08a7, manifest verified; no native build): every read (push, refocus, foreground, 15s poll while valid) goes through one once-only rule (used, seen valid on this device, checked in within 6h, not yet celebrated); brief non-blocking celebration; check-in code untouched. 26 tests incl. live audit checks and mutation checks. STILL OPEN: the fix has not been physically retested, and web/organiser rendered views are unviewed.",
      nextAction: "Needs a FRESH valid ticket (a ticket already used never celebrates, by design; 490a6084 cannot retest). On the attendee phone, on the latest OTA (cold-launch twice): claim a new ticket, open My Ticket so it shows Valid with the QR, then lock the phone. Organiser scans it. Unlock and return: expect a ~2s non-blocking 'You're in!' once, then a settled Used state. Leave and reopen the ticket: no replay. Organiser scans again: 'already used'.",
      lastUpdated: "2026-09-25",
    },
    {
