...
export function SpikSnap({ userId }: { userId: string | null }) {
  ...
  if (phase === "ready") return (
    <GameIntro
      ...
      accent={ACCENT}
      title="Spik Snap"
      description="Does the meaning match the wird? Snap-judge as many as you can in 60 seconds."
      howTo={[
        "You'll see a wird and a meaning",
        "Tap Match if they go together, No match if they don't",
        "Keep going for 60 seconds — wrong answers cost 2 seconds",
      ]}
      onStart={start}
      userId={userId}
      signInPath="/games/spik-snap"
    />
  );
