...
type Row = { id: string; arrived_at: string | null; collected_at: string | null; waiting_fee_pence: number | null; customer_confirmed: boolean | null; created_at: string };
...
export function DisputesManager({ rows }: { rows: Row[] }) {
  ...
  async function resolve(id: string, confirmed: boolean) {
    ...
    try {
      ...
      setList((l) => l.map((r) => (r.id === id ? { ...r, customer_confirmed: confirmed, waiting_fee_pence: confirmed ? r.waiting_fee_pence : 0 } : r)));
      router.refresh();
  ...
  return (
    <div className="space-y-3">
      {list.map((r) => {
        ...
        const isOpen = r.customer_confirmed === null;
        return (
          <Card key={r.id} className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="flex items-center gap-2 font-semibold text-ink">
                Waiting fee dispute
                <StatusPill label={isOpen ? "Open" : r.customer_confirmed ? "Fee confirmed" : "Fee waived"} tone={isOpen ? "amber" : r.customer_confirmed ? "green" : "gray"} />
              </p>
