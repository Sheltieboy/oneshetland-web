...
export const metadata = { title: "Add a memory · OneShetland" };
...
export default async function NewMemoryPage({ searchParams }: { searchParams: Promise<{ parent?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-5 py-10 sm:py-14">
      <Link href="/memories" className="text-sm font-semibold text-ink-soft hover:text-ink">← Memories</Link>
      <div className="mt-4">
        <p className="text-xs font-bold uppercase tracking-widest" style={{ color: MEMORIES }}>OneShetland · Memories</p>
        <h1 className="mt-1 font-display text-4xl font-bold">{parent ? "Add to this memory" : "Pin a memory"}</h1>
