...
export function OffersManager({ businessId, offers }: { businessId: string; offers: LocalOffer[] }) {
  ...
  const [f, setF] = useState({ title: "", description: "", discount_type: "percent" as DiscountType, discount_value: "", valid_until: "", terms: "" });
  ...
  async function create() {
    ...
    try {
      ...
      const { error: e } = await sb.from("local_offers").insert({
        ...
        terms: f.terms.trim() || null, is_active: true,
      });
      if (e) throw e;
      setCreating(false); setF({ title: "", description: "", discount_type: "percent", discount_value: "", valid_until: "", terms: "" }); router.refresh();
          ...
          {offers.map((o) => (
            <div key={o.id} className="flex items-start justify-between gap-3 rounded-card border border-line bg-paper p-4 shadow-soft">
              <div className="min-w-0">
                ...
                <p className="text-sm text-ink-muted">{formatOfferDiscount(o)} · {o.redemption_count} claim{o.redemption_count === 1 ? "" : "s"} · {daysRemaining(o.valid_until)}d left</p>
              </div>
