...
import { requireHubAdmin, hubPayoutReady } from "@/lib/hubs-server";
import { getMembershipTypes } from "@/lib/hubs-data";
import { TiersManager } from "@/components/hubs/admin/TiersManager";
import { hubPayoutHref } from "@/lib/hub-payout-notice";
...
export default async function TiersPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const payoutReady = await hubPayoutReady(hub.id);
  ...
  return (
    <div className="mx-auto max-w-2xl px-5 py-10 sm:py-12">
      ...
      <div className="mt-8"><TiersManager hubId={hub.id} tiers={tiers} accent={accent} payoutReady={payoutReady} payoutHref={hubPayoutHref(hub.slug || hub.id, "tiers")} /></div>
    </div>
  );
}
