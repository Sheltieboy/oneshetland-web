...
export const LAUNCH_READINESS: ReadinessDataset = {
  ...
  items: [
    ...
    },
    {
      ...
      evidence: "VERIFIED IN PRODUCTION AT THE DATA LAYER 25 Sep; RENDERED CHECK STILL OUTSTANDING. Each account read as itself under production row-level security (role authenticated, its own JWT subject) and run through the exact filters the deployed For You (valid/used, upcoming) and My Tickets (valid/used) use, in a transaction that was rolled back (no rows left behind): an account with a cancelled, a refunded and an expired-pending ticket and no owned ticket sees 3 historical rows, 0 For You cards and 0 My Tickets, where the old holder-only rule would have shown 1 false card; an account with a cancelled + a valid ticket for the same event gets exactly 1 card and 1 ticket; a used-only account gets 1 and 1; the real test buyer (real tickets) gets 2 events and 4 tickets. A persistent fixture is now in place: test account bd6276f0 (an owner test account, no paid orders, no Stripe customer) holds one cancelled and one refunded ZZ-marked ticket for the upcoming ZZ TEST Acceptance Event (aebbc34a) and owns nothing there; the event's tickets_sold and quantity_sold were guarded and did not move. Not exercised: the rendered pages, because that needs a signed-in browser session. Earlier history: Launch blocker while unresolved: it falsely tells a customer they own a ticket they have not purchased. Root cause (24 Sep): the web For You query (lib/for-you.server.ts) read event_tickets by holder_id with no status filter, so pending_payment, cancelled and refunded rows showed 'your ticket'; cleanup working did not help because the cancelled row still matched. FIX IMPLEMENTED 24 Sep: one shared OWNED_TICKET_STATUSES (valid, used) in lib/event-ticket-utils.ts, used by For You and web My Tickets; For You now filters status and applies the upcoming-event constraint inside the query. Matches holds_ticket_for(), get_event_social_stats() and mobile My Tickets. Tests: status matrix incl. free claim, contract over every client read of event_tickets, live parity with the DB helpers; mutation-checked. Pushed to main 24 Sep (66090e6). VERIFIED against production data: for the test buyer the old rule would show 'your ticket' on two upcoming events (the ZZ paid-test event, from a cancelled and a pending_payment row — false — and the free acceptance event, from two used tickets — genuine); the new rule leaves only the genuine free-claimed one. NOT yet closed: the deploy of a server-only change leaves no external fingerprint and the rendered strip can only be seen from the buyer's own session, so the on-screen result is still to be confirmed.",
      nextAction: "Physical check, two accounts on the production web site. (1) Sign in as test account bd6276f0: open the homepage For You and My Tickets. Expect NO 'your ticket' card or cue for 'ZZ TEST — Acceptance Event' and NO ticket in My Tickets (the two historical rows exist but grant nothing). (2) Sign in as the test buyer efb83e4b: For You should still show 'your ticket' for the ZZ acceptance events it genuinely owns, and My Tickets should list its valid/used tickets. Both pass → mark complete. Note: the acceptance event starts 26 Sep 10:16 UTC; after that it drops off For You (upcoming filter), so run check (1) before then or the negative result proves nothing.",
      lastUpdated: "2026-09-25",
    },
